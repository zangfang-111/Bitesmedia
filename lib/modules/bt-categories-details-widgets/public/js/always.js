...
*/
    ...
    const makeRequest = function(selectedId, page) {
        var searchText = $('#topic-search').val() || "";
        const query = {};
        ...
        let queryString = '?sort=createdAt&offset=' + (page * limit);
        queryString += searchText ? '&search=' + searchText: '';
    ...
    function changeTopic() {
        ...
        // $(".topic-title-span").html(ti.bt-header__usertle);
        clearResults();
        makeRequest(selectedId, page);
    }
    ...
    $(document).on('keyup', '#topic-search', function(event) {
        clearResults();
        makeRequest(selectedId, page);
    });
